...
class Shape:
    ...
    def get_position_basic(self, position, first, shapelist):
        distances = []
        heights = []
        positions = []
        sbb = self.getBoundingBox()
        for shape in shapelist:
            ...
            distances.append(point_distance(sp, position))
            if self.outOfBounds(sp):
                sp = position
            heights.append(shape.getBoundingBox()[0])
            positions.append(sp)
        if self.outOfBounds(position):
            p = [1, 0, 0, 1, 0, 0]
        elif same_as(position[2], sbb[2] * 0.5):
            p = [0, position[0] - positions[0][0], position[1] - positions[0][1], 0, position[0] - positions[1][0],
                 position[1] - positions[1][1]]

        elif distances[0] < distances[1]:
            if same_as(heights[0], position[2] - sbb[2] * 0.5) and same_as(positions[0][2], heights[0] * 0.5):
                p = [1, position[0] - positions[0][0], position[1] - positions[0][1], 0, position[0] - positions[1][0],
                     position[1] - positions[1][1]]
            elif same_as(heights[0] + heights[1], position[2] - sbb[2] * 0.5):
                if positions[0][2] > positions[1][2]:
                    p = [1, position[0] - positions[0][0], position[1] - positions[0][1], 0,
                         position[0] - positions[1][0],
                         position[1] - positions[1][1]]
                else:
                    p = [0, position[0] - positions[0][0], position[1] - positions[0][1], 1,
                         position[0] - positions[1][0],
                         position[1] - positions[1][1]]
            else:
                p = [0, position[0] - positions[0][0], position[1] - positions[0][1], 1, position[0] - positions[1][0],
                     position[1] - positions[1][1]]

        else:
            if same_as(heights[1], position[2] - sbb[2] * 0.5) and same_as(positions[1][2], heights[1] * 0.5):
                p = [0, position[0] - positions[0][0], position[1] - positions[0][1], 1, position[0] - positions[1][0],
                     position[1] - positions[1][1]]
            elif same_as(heights[0] + heights[1], position[2] - sbb[2] * 0.5):
                if positions[0][2] > positions[1][2]:
    ...
    # "set" functions
    def set_position_basic(self, position, object, shapelist):
        sbb = self.getBoundingBoxWorld()
        if object is None:
            p = [0, 0, sbb[2] * 0.5]
        else:
            op = object.getPosition()
            if position[0] == 0:
                p = [op[0] + position[1], op[1] + position[2], sbb[2] * 0.5]
    ...
    def setPointOrientation(self, orientation, l_index, s_index):
        ...
        if orientation[9] == 1:
            if not frontfacing1:
                ...
                frontfacing2 = s_index
            counter += 1
        if orientation[10] == 1:
            if not frontfacing1:
                frontfacing1 = m_index
            else:
                frontfacing2 = m_index
            counter += 1
    ...
    def intersect(self, plane, line):
        ...
        r = (self.scalarProduct(plane[0], plane[1]) - self.scalarProduct(plane[0], line[0])) / self.scalarProduct(
            plane[0], line[1])
    ...
    def isOnTopOf(self, obb, op, sbb, position):
        ...
        if abs(op[0] - position[0]) <= obb[0] * 0.5 and abs(op[1] - position[1]) <= obb[1] * 0.5 and position[2] > op[
            2] + 0.5 * obb[2] + 0.4 * sbb[2]:
            ontop = True
