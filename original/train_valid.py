...
test_states = test_states.view(n_states, int(n_test_samples / 1), 1, block_sz)
...
if clientID != -1:
    ...
    for epoch in range(2000):
        ...
        print(str(epoch) + " training loss: " + str(mean_loss) + " validation loss: " + str(mean_validation_loss))

        PATH = "state_dict_model_validation_6000samples_attention_net_128_conf10_3_betas07092.pt"
                ...
                for idx in range(3):
                    ...
                    if idx != 0:
                        for bl in range(0, n_blocks):
                            ...
                            state[0][0][bl * state_sz + 18] = o[5]
                            state[0][0][bl * state_sz + 19] = d[0]
                            state[0][0][bl * state_sz + 20] = d[1]
                    ...
                    actionnum = torch.narrow(new_position, 0, 0, 5).view(1, 1, 5)
                    orientationnum = torch.narrow(new_position, 0, 8, 3).view(1, 1, 3)

                    policy1 = torch.narrow(new_position, 0, 5, 3).view(1, 1, 3)
                    policy2 = torch.narrow(new_position, 0, 11, 3).view(1, 1, 3)
                    ...
                    if idx == 0:
                        ...
                        action, _ = ai.action_inference(state.view(1, 1, block_sz), ai_target[-1].view(1, 1, block_sz + 3), new_block, actionnum, orientationnum, False, testing=True)
        ...
        with open('training_progression_0.001_10_betas07092_128.csv', mode='a') as csv_file:
