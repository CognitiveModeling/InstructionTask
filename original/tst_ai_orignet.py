...
PATH = "state_dict_model_4900samples_orig_net_0.00001.pt"
...
if clientID!=-1:
    ...
    for i_batch in range(0, int(n_samples/batch_sz)):
        ...
        for idx in range(3):
            ...
            optimizer1 = torch.optim.Adam([policy1], lr=0.0001)
            optimizer2 = torch.optim.Adam([policy2], lr=0.0001)


            ai = AI.ActionInference(net, policy1, policy2, optimizer1, optimizer2, criterion=net.loss)
            #print(idx)
            print(target)

            if idx == 0:
                #current_block = blocks_target[idx].view(1, batch_sz, n_blocks).to(device="cuda")
                action, _ = ai.action_inference(state, target, new_block, actionnum, orientationnum, True,
                                                current_block=current_best_block)
            else:
                action, _ = ai.action_inference(state, target, new_block, actionnum, orientationnum, False)
