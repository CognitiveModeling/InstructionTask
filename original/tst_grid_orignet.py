...
if clientID!=-1:
    ...
    for i_batch in range(0, int(n_samples/batch_sz)):
        ...
        target_old = states[:, i_batch, :, :]
        target_old = target_old.view(n_states, block_sz)
        ai_target = create_base_target_state(target_old)
        ...
        for idx in range(3):
            ...
            for trial in range(block_tests):
                ...
                if idx == 0:
                    po[0] = torch.tensor([0, 0, 0, 0, 0, 0, 0, 0, orientation_type[0], orientation_type[1], orientation_type[2],
                     facing_choices[0], facing_choices[1]])
                elif p == 0:
                    po[0] = torch.tensor([1, 0, 0, 0, 0, p1, p2, p3, orientation_type[0], orientation_type[1], orientation_type[2],
                     facing_choices[0], facing_choices[1]])
                elif p == 1:
                    po[0] = torch.tensor([0, 1, 0, 0, 0, p1, p2, p3, orientation_type[0], orientation_type[1], orientation_type[2],
                     facing_choices[0], facing_choices[1]])
                elif p == 2:
                    po[0] = torch.tensor([0, 0, 1, 0, 0, p1, p2, p3, orientation_type[0], orientation_type[1], orientation_type[2],
                     facing_choices[0], facing_choices[1]])
                elif p == 3:
                    po[0] = torch.tensor([0, 0, 0, 1, 0, p1, p2, p3, orientation_type[0], orientation_type[1], orientation_type[2],
                     facing_choices[0], facing_choices[1]])
                else:
                    po[0] = torch.tensor([0, 0, 0, 0, 1, p1, p2, p3, orientation_type[0], orientation_type[1], orientation_type[2],
                     facing_choices[0], facing_choices[1]])
                ...
                loss = net.loss(new_state.view(batch_sz, block_sz + 1), target.view(batch_sz, block_sz + 1))
            ...
            final_action = torch.narrow(current_action, 1, 0, 5).view(5)
            ...
            for trial in range(ort_tests):
                ...
                if idx == 0:
                    ...
                    po[0] = torch.tensor(
                        [final_action[0], final_action[1], final_action[2], final_action[3], final_action[4], p1, p2, p3,
                         orientation_type[0], orientation_type[1], orientation_type[2],
                         facing_choices[0], facing_choices[1]])
            ...
            final_ort = torch.narrow(current_action, 1, 8, 3).view(3)
            ...
            for trial in range(fine_tuning):
                ...
                type_choice = torch.argmax(final_ort.view(3)).item()
            ...
            pos = torch.narrow(current_action, 1, 0, 5).view(5)
            leftright = torch.narrow(current_action, 1, 5, 1).item()
            frontback = torch.narrow(current_action, 1, 6, 1).item()
            ort = torch.narrow(current_action, 1, 8, 5).view(n_orientations)
            ...
            sim.simxStartSimulation(clientID, sim.simx_opmode_blocking)
            loss = net.loss(state.view(batch_sz, block_sz), target_old[seq_len].view(batch_sz, block_sz))
        print(loss)
        print(loss.mean())
